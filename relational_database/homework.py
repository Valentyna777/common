...
def task_1_add_new_record_to_db(con) -> None:
    ...
    with con.cursor() as cursor:
        cursor.execute("INSERT INTO customers VALUES(92, 'Thomas', 'David', 'Some Address', 'London', '774',\
                       'Singapore');")
...
def task_12_list_suppliers_from_specified_countries(cur):
    ...
    Returns: 8 records
    """
    cur.execute("SELECT supplierid, suppliername, contactname, city, country FROM suppliers WHERE country IN ('USA',\
                'UK', 'Japan');")
    return cur.fetchall()
...
def task_13_list_products_from_sweden_suppliers(cur):
    ...
    Returns: 3 records
    """
    cur.execute("SELECT products.productname FROM products INNER JOIN suppliers ON\
                 products.supplierid=suppliers.supplierid WHERE country='Sweden';")
    return cur.fetchall()
...
def task_14_list_products_with_supplier_information(cur):
    ...
    Returns: 77 records
    """
    cur.execute("SELECT p.productid, p.productname, p.unit, p.price, s.suppliername, s.country,\
                 s.city FROM products p JOIN suppliers s ON p.supplierid=s.supplierid;")
    return cur.fetchall()
...
def task_15_list_customers_with_any_order_or_not(cur):
    ...
    Returns: 213 records
    """
    cur.execute("SELECT c.customername, c.contactname, c.country, o.orderid FROM customers c FULL JOIN orders o ON\
...
def task_16_match_all_customers_and_suppliers_by_country(cur):
    ...
    Returns: 194 records
    """
    cur.execute("SELECT c.customername, c.address, c.country AS customercountry, s.suppliername, s.country AS\
                 suppliercountry FROM customers c FULL JOIN suppliers s ON c.country=s.country ORDER BY\
